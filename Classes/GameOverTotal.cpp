...
Scene* GameOverTotal::createScene() {
    auto scene = Scene::create();
    auto layer = GameOverTotal::create();
    scene->addChild(layer);
    return scene;
}

bool GameOverTotal::init() {
    if (!Scene::init()) {
        return false;
    }
    ...
    auto restartLabel = Label::createWithTTF("Volver al Inicio", "fonts/Marker Felt.ttf", 36);
